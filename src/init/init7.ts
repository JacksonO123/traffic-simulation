...
import { Car, Road, StopSign, debugCollection, laneLines } from '../engine/road';
...
export const init = (engine: TrafficEngine, canvas: Simulation) => {
  ...
  const roadSpline3 = new Spline2d(vertex(0, 300, 0, laneColor), [
    splinePoint2d(vertex(0, 300), vector2(0, 1), vector2(0, -1))
  ]);
  ...
  intersection.connectMoveRoadEnd(road3, 2);
  ...
  canvas.add(car2);
  car2.setRoute([road2, intersection, road3]);
  // car2.setRoute([road3]);
  // car2.setRoute([road]);
  // car2.setMaxSpeed(1);
  // car2.setMaxSpeed(6);
  ...
  console.log(car2.getRoadData().getRoadOrigins());
